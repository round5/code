...
#define KAT_UNSUITABLE_PARAMETER_SET    -5
...
PQCgetKAT_enc()
{
    ...
    for (int i=0; i<3; i++) {
        for (int j=0; j<25; j++) {
            ...
            mlen = (unsigned long long)(16+i*8);
...
FindMarker(FILE *infile, const char *marker)
{
	...
	while ( 1 ) {
		if ( !strncmp(line, marker, (unsigned long)len) )
			return 1;
...
int main(void) {
#ifdef ROUND5_CCA_PKE
    return PQCgetKAT_enc();
#else
    printf("This parameter set is not suitable for PKE. \n");
    return KAT_UNSUITABLE_PARAMETER_SET;
#endif
}
